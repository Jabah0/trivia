...
def create_app(test_config=None):
    ...
    def add_question():
        ...
        try:
            if search:
                ...
                return jsonify({
                    "questions": current_questions,
                    "totalQustions": len(selection.all()),
                    "currentCategory": "All"
                })
    ...
    def quizzes():
        ...
        try:
            ...
            category = Category.query.filter(Category.type == quiz_category['type']).first()

            if category:
                question = Question.query.filter(Question.id.not_in(previous_questions)).filter(Question.category == category.id).first()
            else:
